<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto mt-6 bg-white dark:bg-gray-800 p-6 rounded-lg shadow transition-colors duration-300">
        <form action="{{ route('terms.update', $term->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <!-- Action Buttons -->
            <div class="flex flex-col sm:flex-row justify-end sm:space-x-2 gap-2">
                <a href="{{ route('terms.index') }}" 
                   class="px-4 py-2 bg-gray-300 dark:bg-gray-700 dark:text-gray-100 rounded-md hover:bg-gray-400 text-center">
                    Cancel
                </a>
