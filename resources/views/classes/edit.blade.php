        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm rounded-lg p-6 font-sans 
                ...
                <form action="{{ route('classes.update', $class->id) }}" method="POST" class="space-y-5">
                    ...
                    {{-- Class Name --}}
                    <div>
                        <label class="block mb-1 font-medium text-gray-700 dark:text-gray-200">Class Name</label>
                    ...
                    <div>
                        ...
                        <select name="form_teacher_id"
                            ...
                            @foreach($teachers as $teacher)
                                <option value="{{ $teacher->id }}" {{ old('form_teacher_id', $class->form_teacher_id) == $teacher->id ? 'selected' : '' }}>
                                    {{ $teacher->user->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="next_class_id"
                            ...
                            @foreach($classes as $c)
                                <option value="{{ $c->id }}" {{ old('next_class_id', $class->next_class_id) == $c->id ? 'selected' : '' }}>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3 pt-3">
                        <button type="submit" 
                                class="px-6 py-2 bg-yellow-600 text-white rounded hover:bg-yellow-700 w-full sm:w-auto">
                            Update Class
                        </button>
